  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="bg-gradient-to-br from-surface-container-lowest to-surface-container border border-outline-variant p-8 rounded-[2.5rem] shadow-xl space-y-8">
        ...
        <motion.button onClick={() => setIsTaskSelectorOpen(true)} className="w-full bg-surface-container p-6 rounded-2xl text-left active:scale-[0.98] transition-all group shadow-md border border-outline-variant" whileHover={{ y: -2 }} whileTap={{ y: 0 }}>
        ...
        <div className="relative flex items-center justify-center py-8">
          <div className="relative w-80 h-80 flex items-center justify-center">
            ...
            <div className="flex flex-col items-center z-10">
              <span className="text-8xl font-black tracking-tighter tabular-nums text-on-surface">{formatTime(timeLeft)}</span>
        ...
        <div className="flex flex-col gap-5">
          <div className="flex gap-5">
            <motion.button onClick={toggleTimer} className={cn("flex-[1.5] py-7 rounded-3xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-3 transition-all shadow-xl", isActive ? "bg-surface-container text-on-surface-variant/60" : "bg-primary text-on-primary shadow-primary/30")} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}>
        ...
        <div className="space-y-5 pt-6 border-t border-outline-variant/50">
          <motion.div className="bg-surface-container p-6 rounded-2xl space-y-4 shadow-md border border-outline-variant" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          ...
          <div className="grid grid-cols-2 gap-5">
            <motion.div className="bg-surface-container p-6 rounded-2xl shadow-md border border-outline-variant" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}>
              <span className="text-[10px] font-black text-on-surface-variant/60 uppercase tracking-widest block mb-4">番茄钟历史</span>
              <div className="flex items-baseline gap-2 mb-3"><span className="text-4xl font-black tracking-tighter">{sessionsToday}</span><span className="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-widest">/ {totalSessionsGoal} 组完成</span></div>
            ...
            <motion.div className="bg-surface-container p-6 rounded-2xl shadow-md border border-outline-variant" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}>
              <span className="text-[10px] font-black text-on-surface-variant/60 uppercase tracking-widest block mb-4">今日专注时长</span>
              <div className="flex items-baseline gap-2 mb-3"><span className="text-4xl font-black tracking-tighter">{focusTimeToday.toFixed(1)}</span><span className="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-widest">小时</span></div>
