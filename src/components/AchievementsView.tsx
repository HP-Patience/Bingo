              ...
              {customAchievements.length > 0 ? (
                <div className="grid grid-cols-3 gap-3">
                  {customAchievements.map(achievement => (
                    <button key={achievement.id} onClick={() => { setSelectedAchievement(achievement); setIsEditing(false); }} className={cn("flex flex-col items-center gap-1.5 transition-all active:scale-95", !achievement.unlocked && "opacity-30")}>
                      <div className={cn("w-full aspect-square rounded-xl flex flex-col items-center justify-center relative transition-all p-2", achievement.unlocked ? "bg-surface-container-lowest border border-outline-variant shadow-sm" : "bg-surface-container-low border border-outline-variant/50")}>
                        <AchievementIcon name={achievement.icon} className="w-8 h-8 text-primary mb-1" />
                ...
                <div className="text-center py-6 bg-surface-container-low/30 rounded-2xl border-2 border-dashed border-outline-variant"><p className="text-on-surface-variant/40 font-bold text-sm italic">暂无自定义成就</p></div>
              )}
              ...
              <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-4 text-center space-y-3 relative overflow-hidden">
                ...
                {!isAddingCustom ? (
                  ...
                  <div className="space-y-4 text-left">
                    <div className="space-y-1.5"><label className="text-[11px] font-semibold tracking-wide text-on-surface-variant px-1">成就名称</label><input type="text" placeholder="成就名称..." className="w-full bg-surface-container-low border border-outline-variant rounded-2xl px-5 py-3 text-sm font-bold outline-none" value={customTitle} onChange={(e) => setCustomTitle(e.target.value)} autoFocus /></div>
                    <div className="space-y-1.5"><label className="text-[11px] font-semibold tracking-wide text-on-surface-variant px-1">获得条件</label><input type="text" placeholder="获得条件 (例如: 连续3天早起)..." className="w-full bg-surface-container-low border border-outline-variant rounded-2xl px-5 py-3 text-sm font-bold outline-none" value={customRequirement} onChange={(e) => setCustomRequirement(e.target.value)} /></div>
                    <div className="flex gap-3 pt-2">
                      <button onClick={() => setIsAddingCustom(false)} className="flex-1 bg-surface-container-low text-on-surface py-3 rounded-2xl font-semibold tracking-wide text-[11px]">取消</button>
                      <button onClick={handleAddCustom} className="flex-1 bg-primary text-on-primary py-3 rounded-2xl font-semibold tracking-wide text-[11px]">创建</button>
                    </div>
                  </div>
                )}
              </div>
            ...
            {isEditing ? (
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5"><label className="text-[11px] font-semibold tracking-wide text-on-surface-variant px-1">成就名称</label><input type="text" className="w-full bg-surface-container-low border border-outline-variant rounded-2xl px-5 py-3.5 text-sm font-bold outline-none" value={editTitle} onChange={(e) => setEditTitle(e.target.value)} /></div>
                  <div className="space-y-1.5"><label className="text-[11px] font-semibold tracking-wide text-on-surface-variant px-1">获得条件</label><input type="text" className="w-full bg-surface-container-low border border-outline-variant rounded-2xl px-5 py-3.5 text-sm font-bold outline-none" value={editRequirement} onChange={(e) => setEditRequirement(e.target.value)} /></div>
                </div>
                <div className="flex gap-3 pt-2">
                  <button onClick={() => setIsEditing(false)} className="flex-1 bg-surface-container-low text-on-surface py-3 rounded-2xl font-semibold tracking-wide text-[11px]">取消</button>
                  <button onClick={handleSaveEdit} className="flex-1 bg-primary text-on-primary py-3 rounded-2xl font-semibold tracking-wide text-[11px]">保存</button>
                </div>
              </div>
            ) : (
              <>
                ...
                <div className="space-y-3">
                  ...
                  <button onClick={() => setSelectedAchievement(null)} className="w-full bg-surface-container-low text-on-surface py-3 rounded-2xl font-semibold tracking-wide text-[11px] active:scale-95 transition-all">关闭</button>
                </div>
              </>
            )}
          </Modal>
