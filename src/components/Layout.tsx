    ...
    <div className="min-h-screen pb-32 max-w-md mx-auto relative overflow-hidden" data-theme={theme}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 w-full max-w-md mx-auto z-50 glass border-t border-outline-variant rounded-t-[2rem] shadow-2xl">
        <div className="flex justify-around items-center px-4 py-3 pb-8">
          <NavItem icon={<Grid className="w-6 h-6" />} label="今日" isActive={activeTab === 'today'} onClick={() => onTabChange('today')} />
          <NavItem icon={<CheckSquare className="w-6 h-6" />} label="任务" isActive={activeTab === 'tasks'} onClick={() => onTabChange('tasks')} />
          <NavItem icon={<CalendarIcon className="w-6 h-6" />} label="日历" isActive={activeTab === 'calendar'} onClick={() => onTabChange('calendar')} />
          <NavItem icon={<Trophy className="w-6 h-6" />} label="成就" isActive={activeTab === 'achievements'} onClick={() => onTabChange('achievements')} />
          <NavItem icon={<ShoppingBag className="w-6 h-6" />} label="商店" isActive={activeTab === 'shop'} onClick={() => onTabChange('shop')} />
          <NavItem icon={<SettingsIcon className="w-6 h-6" />} label="设置" isActive={activeTab === 'settings'} onClick={() => onTabChange('settings')} />
        </div>
      </nav>
    </div>
  );
