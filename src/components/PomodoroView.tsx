    ...
    <div className="space-y-5 pb-6">
      <header className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-headline font-bold text-xl tracking-tight">专注进程</h1>
        </div>
      ...
      <div className="bg-gradient-to-br from-surface-container-lowest to-surface-container border border-outline-variant p-5 rounded-2xl shadow-xl space-y-5">
        <div className="flex bg-surface-container p-2 rounded-3xl border border-outline-variant shadow-md gap-2">
          <button onClick={() => switchMode('work')} className={cn("flex-1 py-4 rounded-2xl text-[10px] font-bold tracking-wide transition-all duration-300", mode === 'work' ? "bg-primary text-on-primary shadow-lg" : "text-on-surface-variant hover:bg-surface-container-high")}>专注</button>
          <button onClick={() => switchMode('shortBreak')} className={cn("flex-1 py-4 rounded-2xl text-[10px] font-bold tracking-wide transition-all duration-300", mode === 'shortBreak' ? "bg-primary text-on-primary shadow-lg" : "text-on-surface-variant hover:bg-surface-container-high")}>短休</button>
          <button onClick={() => switchMode('longBreak')} className={cn("flex-1 py-4 rounded-2xl text-[10px] font-bold tracking-wide transition-all duration-300", mode === 'longBreak' ? "bg-primary text-on-primary shadow-lg" : "text-on-surface-variant hover:bg-surface-container-high")}>长休</button>
        </div>
        ...
        <motion.button onClick={() => setIsTaskSelectorOpen(true)} className="w-full bg-surface-container p-4 rounded-2xl text-left active:scale-[0.98] transition-all group shadow-md border border-outline-variant" whileHover={{ y: -2 }} whileTap={{ y: 0 }}>
          <div className="flex justify-between items-center mb-3"><span className="text-[10px] font-bold text-on-surface-variant/60 tracking-wide">当前任务</span><Edit2 className="w-4 h-4 text-primary opacity-0 group-hover:opacity-100 transition-opacity" /></div>
          <h2 className="text-2xl font-bold tracking-tight text-primary truncate">{selectedTask || '选择一个任务...'}</h2>
        </motion.button>
        ...
        <div className="relative flex items-center justify-center py-6">
          <div className="relative w-56 h-56 flex items-center justify-center">
            ...
            <div className="flex flex-col items-center z-10">
              <span className="text-6xl font-bold tracking-tight tabular-nums text-on-surface">{formatTime(timeLeft)}</span>
              <motion.div className="flex items-center gap-2 mt-4 text-primary" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                ...
                <span className="text-[10px] font-bold tracking-wide">{isActive ? (mode === 'work' ? '工作中' : '休息中') : '已暂停'}</span>
              </motion.div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-5">
          <div className="flex gap-5">
            <motion.button onClick={toggleTimer} className={cn("flex-[1.5] py-4 rounded-3xl font-bold tracking-wide text-xs flex items-center justify-center gap-3 transition-all shadow-xl", isActive ? "bg-surface-container text-on-surface-variant/60" : "bg-primary text-on-primary shadow-primary/30")} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}>
              {isActive ? <Pause className="w-6 h-6" /> : <Play className="w-6 h-6" />}{isActive ? '暂停' : '开始'}
            </motion.button>
            <motion.button onClick={resetTimer} className="flex-1 bg-surface-container text-on-surface py-4 rounded-3xl font-bold tracking-wide text-xs flex items-center justify-center gap-3 transition-all shadow-lg" whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}><RotateCcw className="w-6 h-6" />重置</motion.button>
          </div>
          <motion.button onClick={() => { setIsActive(false); setTimeLeft(durations[mode]); }} className="w-full py-3 rounded-3xl text-[10px] font-bold text-on-surface-variant/50 tracking-wide hover:text-on-surface-variant hover:bg-surface-container transition-all" whileTap={{ scale: 0.98 }}>放弃本次会话</motion.button>
        </div>
        ...
        <div className="space-y-5 pt-6 border-t border-outline-variant/50">
          <motion.div className="bg-surface-container p-4 rounded-2xl space-y-4 shadow-md border border-outline-variant" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            <div className="flex justify-between items-center"><span className="text-[10px] font-bold text-on-surface-variant/60 tracking-wide">今日目标进度</span><span className="text-[10px] font-bold text-primary tracking-wide">{Math.min(100, Math.round((focusTimeToday / 6) * 100))}%</span></div>
            ...
            <div className="flex justify-between text-[10px] font-bold text-on-surface-variant/40 tracking-wide"><span>周目标 ({weeklyGoal}h)</span><span>已完成 {weeklyProgress.toFixed(1)}h</span></div>
          </motion.div>
          <div className="grid grid-cols-2 gap-5">
            <motion.div className="bg-surface-container p-4 rounded-2xl shadow-md border border-outline-variant" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}>
              <span className="text-[10px] font-bold text-on-surface-variant/60 tracking-wide block mb-4">番茄钟历史</span>
              <div className="flex items-baseline gap-2 mb-3"><span className="text-2xl font-bold tracking-tight">{sessionsToday}</span><span className="text-[10px] font-bold text-on-surface-variant/60 tracking-wide">/ {totalSessionsGoal} 组完成</span></div>
            ...
            <motion.div className="bg-surface-container p-4 rounded-2xl shadow-md border border-outline-variant" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}>
              <span className="text-[10px] font-bold text-on-surface-variant/60 tracking-wide block mb-4">今日专注时长</span>
              <div className="flex items-baseline gap-2 mb-3"><span className="text-2xl font-bold tracking-tight">{focusTimeToday.toFixed(1)}</span><span className="text-[10px] font-bold text-on-surface-variant/60 tracking-wide">小时</span></div>
              <motion.div className="text-[10px] font-bold text-primary tracking-wide" animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 2, repeat: focusTimeToday > 0 ? Infinity : 0 }}>{focusTimeToday > 0 ? '+100% vs 昨天' : '开始专注吧'}</motion.div>
            </motion.div>
          </div>
        </div>
      </div>
